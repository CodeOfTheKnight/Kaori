...
func RouterInit() *mux.Router {
	...
	router.Path(endpointEpisode.String()).HandlerFunc(ApiServiceEpisode).Methods(http.MethodGet)
	...
	router.Path(endpointChapter.String()).HandlerFunc(ApiServiceChapter).Methods(http.MethodGet)

	//Rotte API AddData
	routerAdd.Path(addDataMusic.String()).HandlerFunc(ApiAddMusic).Methods(http.MethodPost)
	routerAdd.Path(endpointChapter.String()).Methods(http.MethodGet)
	...
	routerAuth.Path(authRejectSignUp.String()).HandlerFunc(ApiRejectSignUp).Methods(http.MethodGet)
